...
export function AppShell({ children, isAdmin = false, pageTitle, pageSubtitle, bg = 'global' }: AppShellProps) {
  // Known dictionary keys translate; unknown strings pass through unchanged.
  const { t } = useLanguage()
  const title = pageTitle ? t(pageTitle) : pageTitle
  const subtitle = pageSubtitle ? t(pageSubtitle) : pageSubtitle
  return (
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {(title) && (
          <div className="hidden lg:flex items-center justify-between px-8 pt-7 pb-5 border-b border-border-subtle">
            <div>
              <h1 className="font-heading font-bold text-2xl text-text-primary">{title}</h1>
          ...
          <div className="relative z-10">
            ...
            {title && (
              <div className="lg:hidden mb-4">
                <h1 className="font-heading font-bold text-xl text-text-primary leading-tight">{title}</h1>
