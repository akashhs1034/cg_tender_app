...
import type { State, Tender } from '@/lib/mock-data'
import { cn } from '@/lib/utils'

const states: Array<State | 'All'> = ['All', 'Chhattisgarh', 'Uttar Pradesh']
const categories = [...TENDER_CATEGORIES]

export function BidDocumentsClient({ tenders }: { tenders: Tender[] }) {
      ...
      <div className="grid gap-4">
        {tenders.length === 0 ? (
          <div className="text-center py-16 rounded-2xl border border-border-subtle bg-surface">
            ...
            <p className="text-sm text-text-muted mt-1">Use the drafter above to prepare a bid for any tender.</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-16 rounded-2xl border border-border-subtle bg-surface">
            <FileText className="w-8 h-8 text-text-muted mx-auto mb-3" />
                ...
                {t.documentUrl && (
                  <a href={t.documentUrl} target="_blank" rel="noopener noreferrer">
                    <Button size="sm" className="bg-brand-blue hover:bg-brand-blue/90 text-white font-semibold text-xs h-8 gap-1.5">
                      <FileEdit className="w-3.5 h-3.5" /> Open Tender Doc
                    </Button>
                  </a>
                )}
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => toast('Use the drafter above', 'info', { description: `Fill the drafter with ${t.nitNumber} details to generate a bid.` })}
                  className="border-border-subtle text-text-secondary hover:text-text-primary hover:bg-surface-elevated text-xs h-8 gap-1.5"
                >
                  <ClipboardCheck className="w-3.5 h-3.5" /> Draft a Bid
                </Button>
              </div>
            </div>
          ))
